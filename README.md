...
- Генерировались те части кода, в которых находится код, связынный с GigaChat и Resend (отправка emails, работает отправка только на мою почту, потому что сервис бесплатный), а так же использовал для генерации Dockerfile, supervisor и nginx для деплоя
...
- Rate limiting организованы через middleware throttle на маршрут (ограничение 30 запросов в минуту)
